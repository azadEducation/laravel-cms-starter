...
class Website extends BaseModel
{
    ...
    {
        if (self::has($key)) {
            $setting = self::getAllWebsites()->where('name', $key)->first();
    ...
    {
        return Cache::rememberForever('websites.all', function () {
            return self::all();
        });
    }
    ...
    {
        return collect(config('wesbite_fields'))->pluck('elements')->flatten(1);
    }
